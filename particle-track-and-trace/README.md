...
data/
  ...
  hydrodynamic_V.h5
  spawn_locations.csv
  GenerateDistanceMap.py
  shore_distance.csv (generated by the script in the data folder from the GEBCO_2024_CF.nc file)
...
```
Then run 
interactive-track-and-trace\particle-track-and-trace\src\build\Release\ParticleTrackTrace.exe from the file explorer.
...
- `plot_all_particles.py` plots the trajectories of all particles.
- `plot_particle.py` plots the trajectory of a selected particle.
- `curvature_analysis.py` plots the curvature of the trajectory of a selected particle.