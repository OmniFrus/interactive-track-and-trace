#include "ParcelsAdvectionConditionKernel.h"
...
ParcelsAdvectionKernel::ParcelsAdvectionKernel(std::unique_ptr<RK4AdvectionKernel> baseKernel, std::shared_ptr<UVGrid> grid)
    : baseKernel(std::move(baseKernel)), grid(grid) {}

std::pair<double, double> ParcelsAdvectionKernel::advect(int time, double lat, double lon, int dt) const {
...
//    }
    ...
    auto applySlip = [&](double u, double v, double lat, double lon) -> Vel {
        ...
        const double slipRatio = 1; // Full slip; use <1.0 for partial slip
