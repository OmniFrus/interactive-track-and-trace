#include "FreeSlipAdvectionConditionKernel.h"
...
FreeSlipAdvectionConditionKernel::FreeSlipAdvectionConditionKernel(
    std::unique_ptr<AdvectionKernel> baseKernel,
    std::shared_ptr<UVGrid> grid)
    : baseKernel(std::move(baseKernel))
    , grid(std::move(grid)) {}

std::pair<double, double> FreeSlipAdvectionConditionKernel::advect(int time, double latitude, double longitude, int dt) const {
