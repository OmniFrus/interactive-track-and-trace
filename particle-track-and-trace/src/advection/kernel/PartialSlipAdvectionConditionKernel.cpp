#include "PartialSlipAdvectionConditionKernel.h"
#include "../interpolate.h"
#include <cmath>
#include <algorithm>

PartialSlipAdvectionConditionKernel::PartialSlipAdvectionConditionKernel(
    std::unique_ptr<AdvectionKernel> baseKernel,
    std::shared_ptr<UVGrid> grid)
    : baseKernel(std::move(baseKernel))
    , grid(std::move(grid)) {}

std::pair<double, double> PartialSlipAdvectionConditionKernel::advect(int time, double latitude, double longitude, int dt) const {
